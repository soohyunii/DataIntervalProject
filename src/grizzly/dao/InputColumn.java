...
public class InputColumn {
	...
	public static void main(String[] args) {
		...
		tableName=jpanel03.tableName;
		System.out.print("조사할 테이블 명을 입력하세요 : "+tableName);
		PkValue=jpanel03.tablePk;
		System.out.print("테이블의 PK값을 입력하세요 : "+PkValue);
		
		do {
			System.out.print("유격조사할 컬럼명을 입력하세요 : ");
			colName=scan.nextLine();
			// 이부분 do-while문으로 고치면 중복 입력을 안해도 되지 않을까? - exit를 입력하면 빠져나가도록
			
			
			try {
				String totalCount ="SELECT COUNT(*) FROM TEMP1_"+colName;
				String quary="CREATE TABLE TEMP1_"+colName+" AS (SELECT "+colName+", "+PkValue+" AS ORI_PK FROM "+tableName+" WHERE "+colName+" IS NOT NULL) ORDER BY "+colName;
				String quary2="ALTER TABLE TEMP1_"+colName+" ADD (PK1 NUMBER)";
				String quary3="UPDATE TEMP1_"+colName+" SET PK1=ROWNUM";
				String quary4="CREATE TABLE TEMP2_"+colName+" AS (SELECT "+colName+", "+PkValue+" AS ORI_PK FROM "+tableName+" WHERE "+colName+" IS NOT NULL) ORDER BY "+colName;
				String quary5="ALTER TABLE TEMP2_"+colName+" ADD (PK2 NUMBER)";
				String quary6="UPDATE TEMP2_"+colName+" SET PK2=ROWNUM+1";
				String quary7="CREATE TABLE DIST_NEXT_"+colName+" AS SELECT ROWNUM AS PK, DATA.* "
						+ "FROM (SELECT (A."+colName+" - B."+colName+") AS DIST_NEXT "
								+ "FROM TEMP1_"+colName+" A, TEMP2_"+colName+" B WHERE A.PK1=B.PK2 ORDER BY A.PK1) DATA";
				...
				while(rs8.next()) {
					...
					System.out.println("DIST_NEXT 테이블에 INSERT VALUES("+result+",0)");			
				}
				...
				while(rs13_1.next()) {
					...
					System.out.println("DIST_PREV 테이블에 "+result+"값 삭제");
				}
				...
				System.out.println("최종 테이블까지 전부 생성되었습니다. 'FINAL_"+colName+"' 테이블을 확인하고 ORI_PK키로 원본과 JOIN해 삭제하세요.");
				System.out.println();
				System.out.println("다음 유격조사를 바로 진행합니다");
				System.out.println("종료를 원하면 exit를 입력해주세요");
				System.out.println();
		...
		} while (!tableName.isEmpty()|!PkValue.isEmpty()|!colName.equals("exit"));
			try {
				if(rs!=null) {rs.close();}
